...
class People:
    def __init__(self=None, name=None, country=None, date=None, ):
        self.name = name
        if country is None:
            self.country = []
        if date is None:
    ...
    def from_dict(self, data):
        for field in ['name', 'date', 'country']:
            if field in data:
...
class Mock_Tech:
    ...
    def add(cls, People_name):
        ...
        try:
            ...
            r = requests.get(
                f'https://ct-mock-tech-assessment.herokuapp.com/{People_name.lower()}').json()
            print(r)
            p = People()
            data_dict = {
                'name': r["partners"]["firstName"]["lastName"].title(),
            ...
            print(error)
            input('There was an error populating your Mock_Tech. Please try again.')
         #add new People
